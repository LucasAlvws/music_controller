...
class UpdateView(APIView):
    ...
    def pacth(self, request, format=None):
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            ...
            result_room = Room.objects.filter(code=code)
            if not result_room.exists():
                return Response({'Room not found': 'invalid room...'}, status=status.HTTP_404_NOT_FOUND)
            room = result_room[0]
            user_id = self.request.session.session_key
            if user_id != room.host:
                return Response({'msg': 'You are note the host room...'}, status=status.HTTP_403_FORBIDDEN)
            room.guest_can_pause = guest_can_pause
            room.votes_to_skip = votes_to_skip
            room.save(update_fields=['guest_can_pause', 'votes_to_skip'])
            return Response(RoomSerializer(room).data, status=status.HTTP_200_OK)
